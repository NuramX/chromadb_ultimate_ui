...
|---|---|---|
| `CUI_BATCH` | `50` | จำนวน record ที่ดึงจาก source ต่อ 1 request |
| `CUI_WRITE_FACTOR` | `1` | สะสมกี่ batch ก่อนเขียนลง target ครั้งเดียว (1 = เขียนเท่า CUI_BATCH; เพิ่มถ้า target รับ upsert ใหญ่ได้) |
| `CUI_PREFETCH_DEPTH` | `2` | reader ดึงล้ำหน้า writer ได้กี่ batch (ยิ่งน้อยยิ่งปลอดภัยกับ source ที่ไม่แรง) |
...
ค่าเหล่านี้ปรับได้ทั้งผ่าน environment variable หรือแก้ตรงๆ ใน [`backend/app/config.py`](backend/app/config.py)
...
2. แบ่ง IDs เป็น slice ขนาด `CUI_BATCH` → ดึงข้อมูลด้วย `get(ids=slice)` ทีละชุด
3. ใช้ pipeline 2 thread: **reader** ดึง batch ถัดไปจาก source ขณะที่ **writer**
   เขียน batch ก่อนหน้าลง target พร้อมกัน — ทับเวลารอ network ของทั้งสองฝั่ง
   (source ยังถูกดึงทีละ 1 request เท่าเดิม ไม่หนักขึ้น)
4. writer เขียนลง target ด้วย `upsert` (idempotent — รันซ้ำได้ปลอดภัย)
5. บันทึก progress + checkpoint ลง SQLite หลังทุกครั้งที่เขียน target สำเร็จ
6. Jobs รันทีละ 1 งาน (FIFO queue) — server ต้นทางไม่ถูก hit พร้อมกัน
7. เมื่อ backend restart → jobs ที่ค้างอยู่ถูก re-enqueue อัตโนมัติ
